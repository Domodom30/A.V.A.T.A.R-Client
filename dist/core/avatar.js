...
async function HTTPconnect(serverIp) {
	...
	return new Promise((resolve) => {
		...
		.on('connect', () => {
			socket.emit('client_connect', Config.client, ipClient, Config.http.client.port, Config.speech.server_speak, Config.listen.loop_mode, null , Config.speech.locale);
		})
		.on('disconnect', () => {
...
async function copyIntercomFile(stream, callback) {
	let file = path.join(__dirname, 'intercom', 'intercom.wav');
