...
if (prop === null) {
    prop = fs.readJsonSync(path.resolve(__dirname, '../assets/config/default/Avatar.prop'), { throws: false });
}

const soundexfr = await import('@jollie/soundex-fr');
const soundexen = await import('soundex-code');
const soundex = (prop.speech.locale === 'fr-FR') ? soundexfr.default : soundexen.default;
...
function changeVoice() {
    ...
    if (Config.speech.server_speak) {
      ...
      if ((Config.voices.active[Config.voices.type].indexOf(Config.voices.current[Config.voices.type]) === -1) 
        || (Config.voices.active[Config.voices.type].indexOf(Config.voices.current[Config.voices.type]) === (Config.voices.active[Config.voices.type].length - 1))) 
      {
        Config.voices.current[Config.voices.type] = Config.voices.active[Config.voices.type][0];
      } else {
        Config.voices.current[Config.voices.type] = Config.voices.active[Config.voices.type][Config.voices.active[Config.voices.type].indexOf(Config.voices.current[Config.voices.type]) + 1];
      }
      
      //just in case of...
      if (!Config.voices.current[Config.voices.type]) Config.voices.current[Config.voices.type] = "by default";
...
let initProperties
async function getRemoteVoices(init) {
    initProperties = init;
    await textToSpeech.getVoices();
}
...
async function manageActions (buffer, aka) {
    ...
    if (buffer.toLowerCase() === Config.voices.change.toLowerCase()) {
        return changeVoice();
    }
...
function findSpeech(sentence, answers, tbl_answers) {
	...
	if (!tts && (Config.speech.locale.indexOf('fr-') > -1 || Config.speech.locale.indexOf('en-') > -1)) {
...
function isGrammar(sentence, rules) {
    ...
    return new Promise((resolve) => {
        ...
        if (Config.speech.locale.indexOf('fr-') > -1 || Config.speech.locale.indexOf('en-') > -1) {
            var sdx = soundex(sentence, sentence.length);
            var score = 0;
            for (var i=0; i < rules.grammar.length; i++){
