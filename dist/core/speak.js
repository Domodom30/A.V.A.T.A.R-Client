...
async function speak(tts, callback, end, voice, volume, speed, pitch, test) {
    ...
    setTimeout(async () => {
        ...
        let type, options = {text: tts};
        if (voice)
          options.voice = voice;
        else if (!test && Config.voices.current[Config.speech.locale] && Config.voices.current[Config.speech.locale][Config.voices.type[Config.speech.locale]].toLowerCase() !== "by default") {
          options.voice = Config.voices.current[Config.speech.locale][Config.voices.type[Config.speech.locale]];
          type = Config.voices.type;
        } 
        ...
        if (volume)
          ...
          options.volume = Config.voices.volume[Config.speech.locale][Config.voices.type[Config.speech.locale]];

        if (speed)
          options.speed = speed;
        else if (Config.voices.speed)
          options.speed = Config.voices.speed[Config.speech.locale][Config.voices.type[Config.speech.locale]];
