...
# Remove old files
Write-Host "⏳​ Removing old files" -NoNewline -ForegroundColor DarkMagenta
...
$installType = $null
$del = $null
If ((Test-Path "./download/output/A.V.A.T.A.R-Client") -eq $True) {
...
Write-Host "⏳​ Copying new version to the A.V.A.T.A.R client directory, please wait..." -NoNewline -ForegroundColor DarkMagenta
...
# Update package.json
Write-Host "⏳​ Update package.json" -NoNewline -ForegroundColor DarkMagenta
