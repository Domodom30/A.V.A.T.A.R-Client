...
function setProperties() {
  ...
  BCP47dialog = fs.readJsonSync(path.resolve(__dirname, 'locales/BCP47-dialog.loc'), { throws: false });
  BCP47app = fs.readJsonSync(path.resolve(__dirname, 'locales/BCP47-app.loc'), { throws: false });
}
...
async function testVoice (arg) {
  ...
  }, true, arg.voice, arg.volume, arg.speed, arg.pitch, true);
}
...
async function openSettings(init, webVoices) {
  ...
  const tblvoices = {"web-voice": webVoices, "local-voice": voices};
...
const checkUpdate = async () => {
  if (Config.checkUpdate === true) {
    const result = await github.checkUpdate(mainWindow, Config);
    if (result !== false) {
