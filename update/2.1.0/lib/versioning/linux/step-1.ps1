...
Write-Host "⏳​ Downloading client master package from GitHub" -NoNewline 
...
if (Test-Path ./newVersion-$version) {
    Write-Host "⏳​ Removing old existing client master package" -NoNewline 
    Remove-Item ./newVersion-$version -Recurse -Force
    Write-Host " done" -ForegroundColor Green
}
Write-Host "⏳​ Unziping client master package" -NoNewline 
...
If (($installType -eq "exe") -or ($installType -eq "module")) {
    ...
    If ($installType -eq "exe") {
        ...
        start-process -FilePath "npm" -ArgumentList "install", "--save-dev @electron/packager" -NoNewWindow -workingdirectory "$package" -Wait 
        Write-Host "Electron packager installed" -ForegroundColor Green
        Start-Sleep -Seconds 1
        Write-Host "⏳​ Creating a new A.V.A.T.A.R client application, please wait..." 
...
Write-Host "⏳​ Creating ZIP file, please await... " -NoNewline 
...
$ErrorActionPreference = 'Stop'
Write-Host "⏳​ Restarting the A.V.A.T.A.R client" -NoNewline 
$app_location = "$current_location/../../.."
start-process -filePath "$app_location/A.V.A.T.A.R-Client" -workingdirectory "$app_location" -wait
Write-Host " done" -ForegroundColor Green
Start-Sleep -Seconds 1
